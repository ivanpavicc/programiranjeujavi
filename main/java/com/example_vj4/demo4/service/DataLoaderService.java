...
public class DataLoaderService {
    ...
    @Autowired
    public DataLoaderService(BookRepository bookRepository, MemberRepository memberRepository) {
    ...
    public void loadMembers() {
        try {
            // Učitavanje JSON datoteke
            ObjectMapper objectMapper = new ObjectMapper();
            InputStream inputStream = getClass().getClassLoader().getResourceAsStream("members.json");
            List<Member> members = objectMapper.readValue(inputStream, new TypeReference<List<Member>>() {});

            // Spremanje članova u bazu podataka
            memberRepository.saveAll(members);

            System.out.println("Podaci su uspješno učitani iz members.json u bazu podataka.");
        } catch (Exception e) {
            e.printStackTrace();
            System.err.println("Došlo je do greške prilikom učitavanja podataka iz members.json.");
        }
    }
}
